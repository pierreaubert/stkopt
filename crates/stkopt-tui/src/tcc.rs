...
//!
//! Handles camera permission checks and requests for terminal apps.
//! macOS TCC uses bundle IDs to identify apps, so we map terminal programs
//! to their corresponding bundle IDs.
...
const TCC_DB_PATH: &str = ""; // Not used on other platforms

const CAMERA_SERVICE: &str = "kTCCServiceCamera";

pub fn get_terminal_bundle_id() -> (&'static str, &'static str) {
    ...
    let (bundle_id, display_name) = match term_program.as_str() {
        ...
        _ => {
            tracing::warn!(
                "Unknown terminal program: {}, defaulting to Terminal",
                term_program
            );
...
pub fn check_camera_permission() -> Result<Option<bool>, String> {
    ...
    let (bundle_id, display_name) = get_terminal_bundle_id();
...
#[cfg(target_os = "macos")]
pub fn print_camera_permission_status() {
    let (bundle_id, display_name) = get_terminal_bundle_id();
    tracing::info!(
        "Terminal detected: {} (bundle ID: {})",
        display_name,
        bundle_id
    );
    ...
    match check_camera_permission() {
        ...
        Ok(Some(false)) => {
            ...
            tracing::warn!(
                "To grant camera access, go to System Settings → Privacy & Security → Camera"
            );
            tracing::warn!("Select {} and enable camera access", display_name);
        }
        Ok(None) => {
            tracing::info!("Camera permission: NOT SET (prompt will appear on first use)");
        }
        Err(e) => {
            tracing::warn!("Could not check camera permission: {}", e);
        }
    }
}
...
#[cfg(not(target_os = "macos"))]
pub fn print_camera_permission_status() {
    // Camera permission is not applicable on non-macOS platforms
}
...
mod tests {
    ...
    #[test]
    fn test_terminal_bundle_id_mapping() {
        // Test that terminal mapping works
        let (bundle_id, _) = get_terminal_bundle_id();
        assert!(bundle_id.contains(".") || bundle_id == "com.apple.Terminal");
    }

    #[test]
    #[cfg(target_os = "macos")]
    fn test_check_camera_permission() {
        // This test will only pass on macOS with TCC.db accessible
        // The function may return Err if TCC.db is not accessible
