                                        ...
                                        match validate_address(&input) {
                                            ValidationResult::Valid(_) => {
                                                    ...
                                                    && let Some(ref handle) = this.chain_handle
                                                {
                                                    ...
                                                    let pending = this.pending_updates.clone();
                                                    Tokio::spawn(cx, async move {
                                                        match handle.fetch_account(address).await {
                                                            Ok(account_data) => {
                                                                if let Ok(mut queue) = pending.lock() {
                                                                    queue.push(ChainUpdate::AccountLoaded(account_data));
                                                                }
                                                            }
                                                            Err(e) => {
                                    ...
                                    .on_click(move |_window, cx| {
                                        ...
                                        entity.update(cx, |this, cx| {
                                                ...
                                                && let Some(ref handle) = this.chain_handle
                                            {
                                                ...
                                                let pending = this.pending_updates.clone();
                                                Tokio::spawn(cx, async move {
                                                    match handle.fetch_account(addr).await {
                                                        Ok(account_data) => {
                                                            if let Ok(mut queue) = pending.lock() {
                                                                queue.push(ChainUpdate::AccountLoaded(account_data));
                                                            }
